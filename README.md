...
Generation is local-only. Trigger.dev does not run OpenCode and there is no encoded cloud credential path. The generation agent uses OpenCode’s blanket `permission: "allow"` setting, including bash, file edits, and questions. Generation can run on macOS or Linux. Each run uses a temporary working directory and a one-hour timeout.
