...
import { parseOpenCodeNextArguments } from "./opencode-next-args";

const CLAUDE_NEXT_USAGE =
  "Usage: bun run claude:next -- [count | --count count | -n count | --dry-run]";
...
async function preflight(runtime: ClaudeCodeRuntime) {
  const details = await runtime.preflight();

  console.log(
    `${tag("OK", "green")} Preflight passed ${dim(
      `Claude Code ${details.cliVersion} (SDK ${details.sdkVersion}), ${details.resolvedModel} at ${details.effort} effort, auth ${details.apiKeySource}, displayed as ${details.displayName}`,
    )}`,
  );

  if (process.env.ANTHROPIC_API_KEY?.trim()) {
    console.warn(
      `${tag("WARN", "yellow")} ANTHROPIC_API_KEY is set, so this run may bill Anthropic API credits rather than your Claude Code subscription.`,
    );
  }
...
async function main() {
  ...
  const runtime = await createLocalClaudeCodeRuntime();
  ...
  try {
    ...
    try {
      ...
      const reason = `Local Claude Code generation failed: ${
